...
# img = sp.misc.imresize(im2, 0.50)
#  / 255.
# print img.shape

# Convert the image into a graph with the value of the gradient on the edges.
imf = np.float64(im2)
# graph = image.img_to_graph(imf)
# print graph.shape   # (4608,4608)
...
lab = sc.labels_.astype(np.int)
print lab.shape

# # cluster = sc.fit_predict(img)
# # print cluster.shape
# # cluster = cluster.reshape(321, 481)
# lab = lab.reshape(321, 481)
# plt.imshow(lab)
# plt.colorbar()
# plt.show()
...
# imh2 = np.transpose(imh2)
# pca = PCA(n_components=3)
# pca.fit(imh2)
# # print pca.components_.shape
# inputimh = np.transpose(pca.components_)
# gmm2 = GaussianMixture(n_components=8, covariance_type="tied")
# gmm2 = gmm2.fit(inputimh)
#
# cluster = gmm2.predict(inputimh)
# cluster = cluster.reshape(610, 340)
# plt.imshow(cluster)
# print("--- %s seconds ---" % (time.time() - start_time))
# plt.colorbar()
# plt.show()
# --- 26.4172759056 seconds ---
# can work, but slow
...
# # print pca.components_.shape
# inputimh = np.transpose(pca.components_)
# kmeansh = KMeans(n_clusters=8, random_state=0).fit(inputimh)
...
# plt.colorbar()
# plt.show()
# --- 18.9854960442 seconds ---
# faster than not using PCA
